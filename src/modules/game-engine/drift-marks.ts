...
class DriftTrail {
  ...
  track(wheel: THREE.Object3D | null, groundY: number, intensity: number, emit: boolean) {
    if (!wheel) return;
    ...
    if (emit && this.active) {
      ...
      this.writeSegment(this.prev, _wheelWorld, alpha, true);
    }
  ...
  private writeSegment(prev: THREE.Vector3, curr: THREE.Vector3, alpha: number, markDirty: boolean) {
  ...
  load(strokes: Stroke[]) {
    for (const s of strokes) {
      ...
      for (let n = 0; n < segs; n++) {
        ...
        this.writeSegment(_replayPrev, _replayCurr, intensities[n] / 255, false);
...
export class DriftMarks {
  private trails: [DriftTrail, DriftTrail];
  private storageKey: string;
  private handlePageHide: () => void;

  constructor(scene: THREE.Scene, trackId: string | null) {
    ...
    this.trails = [new DriftTrail(scene, material), new DriftTrail(scene, material)];

    this.storageKey = STORAGE_PREFIX + (trackId || "default");
    this.load();

    this.handlePageHide = () => this.save();
  ...
  private load() {
    try {
      ...
      // corrupted/unavailable storage -- start with empty trails
    }
  }
  ...
  private save() {
    ...
    try {
      ...
      // storage unavailable/quota exceeded -- marks just won't persist
    }
  }
